...
(poprzedni mechanizm, zachowany dla~kompatybilności) oraz
korzenie Merkle dla partii. Kluczowe jest mapowanie O(1)
...
Tylko SSL/TLS  & Tak  & Nie   & $\sim$TLS  & 0           \\
Blockchain RPC & Tak  & Tak   & 300--500   & $\sim$94\,000 \\
Merkle offline & \textbf{Nie} & \textbf{Tak} &
                 \textbf{$\sim$12} & \textbf{$<$5\,000} \\
\bottomrule
\end{tabular}
\end{table}
...
Czas~12\,ms w~scenariuszu~4 obejmuje obliczenie SHA-256 pliku
GLB oraz pełną rekonstrukcję korzenia Merkle w~przeglądarce.
W~scenariuszu~3 dominującym składnikiem jest opóźnienie
sieciowe zapytania RPC do~węzła Ethereum (300--500\,ms),
niezależne od~rozmiaru pliku. Przyspieszenie jest zatem ponad
trzydziestokrotne.

\subsection{Analiza kosztów gazu}

Pomiary wykazały, że~\texttt{registerModel} kosztuje
średnio~93\,997~jednostek gazu. Wywołanie
\texttt{registerMerkleRoot} dla~dowolnej partii modeli kosztuje
około~170\,000~gazu niezależnie od~$N$. Przy 100~modelach
koszt jednostkowy spada do~około~1\,700~gazu/model, co~stanowi
redukcję o~ponad~98\% w~porównaniu z~rejestracją
indywidualną. Efekt ten wynika z~amortyzacji stałych kosztów
transakcji (21\,000~gazu bazowego) i~pojedynczego zapisu
\texttt{bytes32} na~blockchainie; jedynym kosztem skalowalnym
liniowo pozostaje aktualizacja mapowania
\texttt{modelToBatchId}.
